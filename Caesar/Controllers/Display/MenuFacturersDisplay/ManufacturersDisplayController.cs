...
using PagedList;
using PagedList.Mvc;
namespace Caesar.Controllers.Display.MenuFacturersDisplay
{
    public class ManufacturersDisplayController : Controller
    {
        ...
        public ActionResult ListMenufacturers(int? page,string tag)
        {
            ...
            ViewBag.Title = "<title>Hệ thống phân phối Caesar</title>";
            ViewBag.Description = "<meta name=\"description\" content=\"Hệ thống phân phối chính hãng sản phẩm thiết bị vệ sinh Caesar trên toàn quốc. Địa chỉ : 391 - Nguyễn Xiển - Thanh Xuân - HN\"/>";
            ViewBag.Keyword = "<meta name=\"keywords\" content=\"Hệ thống phân phối thiết bị vệ sinh Caesar\" /> ";
            if (tag != "" && tag != null)
            {
                int idMenu;
                string Chuoi = tag;
        ...
        public ActionResult ManufacturersDetail(string tag)
        {
            ...
            {
                chuoinew += "<a href=\"/3/" + listnew[i].Tag + "-" + listnew[i].id + ".aspx\" title=\"" + listnew[i].Name + "\"> - " + listnew[i].Name + "</a>";
            }
